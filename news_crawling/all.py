...
for i in tqdm(final_urls):
    ...
    # 뉴스 공감 수 가져오기(셀레늄)
    if parts == "TV연예":
        likes = str(news2_html.select("#content > div.end_ct > div > div.end_top_util > div:nth-child(1) > div._reactionModule.u_likeit > a > span.u_likeit_text._count.num")).replace('<span class="u_likeit_text _count num">',"").replace("</span>", "").replace("[", "").replace("]", "")
    elif parts == "스포츠":
        likes = str(news2_html.select("#content > div > div.content > div > div.news_work > div.count > div._reactionModule.u_likeit > a > span.u_likeit_text._count.num")).replace('<span class="u_likeit_text _count num">',"").replace("</span>", "").replace("[", "").replace("]", "")
    else:
        likes = str(news2_html.select("#commentFontGroup > div.media_end_head_info_variety_likeit._LIKE_HIDE.as_likeit_improve > div > a > span.u_likeit_text._count.num")).replace('<span class="u_likeit_text _count num">',"").replace("</span>", "").replace("[", "").replace("]", "")
    
    if likes == "":
        news_like.append("0")
    else:
        news_like.append(likes)
    ...
    if parts != 'TV연예' or parts != '스포츠':
        new_url = 'https://n.news.naver.com/mnews/article/comment/'+ i[39:]
        ...
        while True:
            ...
            try:
                more = driver.find_element(By.CSS_SELECTOR, '#cbox_module > div.u_cbox_wrap.u_cbox_ko.u_cbox_type_sort_new > div.u_cbox_paginate > a > span > span > span.u_cbox_page_more')
                more.click()
                time.sleep(0.1)
...
print('comment_user: ', list_nicknames)
print('comment_content: ', list_contents)
print('comment_like: ', list_likes)
print('comment_dislike: ',list_dislikes)
print('comment_datetime: ', list_datetimes)

# step3. 그대로 dbeaver로

import pymysql

connection = pymysql.connect(host='localhost', user='root', port = 3306, password = "clftjd4dlek!", db = 'third_project', charset = 'utf8mb4', use_unicode=True)
cur = connection.cursor()

for news_id, news_title, news_url, news_content, news_date, news_type, news_img, news_like in zip(news_ids, news_titles, final_urls, news_contents, news_dates, news_types, news_img, news_like):
...
for news_id, nick, content, like, dislike, created_at in zip(news_ids, list_nicknames, list_contents, list_likes, list_dislikes, list_datetimes):
    
    sql = """INSERT INTO main_page_newscomment (news_number, username, comment_content, comment_like, comment_dislike, created_at) VALUES(%s, %s, %s, %s, %s, %s)"""
    cur.execute(sql, ('%s'%(news_id),'%s'%(nick), '%s'%(content), '%s'%(like), '%s'%(dislike), '%s'%(created_at)))
    connection.commit()
